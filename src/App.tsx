...
import type { AchievementRecord, DailyHabit, ExerciseDefinition, ExerciseLog, PainStatus, PlannedSession, SetLog, TimerState, WaistEntry, Walk, WeeklyReview, WeightEntry, WorkoutSession, WorkoutTemplate } from "./types";
...
import { formatPace, monthSummary, paceMinutesPerKm, speedKmPerHour, summarizeWeek } from "./lib/calculations";
...
function Strength() {
  const data = useData();
  return <Page title="Strength" subtitle="Edit the active program, reschedule sessions, and reopen completed workouts.">
    <div className="grid gap-4 lg:grid-cols-2"><section className={card}><h2 className="text-xl font-bold">Month 1 schedule</h2><div className="mt-3 space-y-3">{data.planned.map((planned) => <PlannedSessionRow key={planned.id} planned={planned} templates={data.templates} />)}</div></section><WorkoutEditor data={data} /></div>
    <section className={`${card} mt-4`}><h2 className="text-xl font-bold">Completed and active sessions</h2><div className="mt-3 space-y-3">{data.sessions.map((session) => <div key={session.id} className="flex items-center justify-between gap-3 rounded-2xl bg-white/5 p-3"><div><p className="font-bold">{session.workoutName}</p><p className="text-sm text-slate-300">{session.date} · {session.status}</p></div><div className="flex gap-2"><Link className={ghost} to={`/strength/${session.id}`}><Pencil size={16} /> Edit</Link><button className={ghost} onClick={() => deleteSession(session.id)}><Trash2 size={16} /> Delete</button></div></div>)}</div></section>
  </Page>;
}

function PlannedSessionRow({ planned, templates }: { planned: PlannedSession; templates: WorkoutTemplate[] }) {
...
function ExerciseLogCard({ log, sets, onRest }: { log: ExerciseLog; sets: SetLog[]; onRest: (seconds: number) => void }) {
  ...
  return <section className={card}><h2 className="text-xl font-bold">{log.name}</h2><p className="text-sm text-slate-300">{log.category} · {log.type} · rest {log.restSeconds}s</p><p className="mt-1 text-sm text-slate-400">{log.instructions}</p>{painCount >= 2 && <p className="mt-2 rounded-2xl border border-amber-300/40 bg-amber-400/10 p-3 text-sm text-amber-100">This exercise has caused discomfort more than once. Replace or review it before repeating.</p>}<div className="mt-3 space-y-2">{sets.sort((a, b) => a.setNumber - b.setNumber).map((set) => <SetRow key={set.id} set={set} restSeconds={log.restSeconds} onRest={onRest} />)}</div></section>;
}
...
function SetRow({ set, restSeconds, onRest }: { set: SetLog; restSeconds: number; onRest: (seconds: number) => void }) {
  ...
  return <div className="grid gap-2 rounded-2xl bg-white/5 p-2 sm:grid-cols-[3rem_1fr_1fr_1fr_1fr_auto]"><span className="self-center font-bold">Set {set.setNumber}</span><input aria-label="Weight" className={input} value={set.weight} onChange={(event) => update({ weight: event.target.value })} /><input aria-label="Reps" className={input} value={set.reps} placeholder="reps" onChange={(event) => update({ reps: event.target.value })} /><input aria-label="RIR" className={input} value={set.rir} placeholder="RIR" onChange={(event) => update({ rir: event.target.value })} /><select aria-label="Pain status" className={input} value={set.painStatus} onChange={(event) => update({ painStatus: event.target.value as PainStatus })}><option value="none">No pain</option><option value="mild">Mild discomfort</option><option value="stopped-set">Pain, stopped set</option><option value="stopped-exercise">Pain, stopped exercise</option><option value="stopped-workout">Pain, stopped workout</option></select><button className={set.completed ? ghost : button} onClick={toggle}>{set.completed ? "Done" : "Complete"}</button><input aria-label="Set note" className={`${input} sm:col-span-6`} value={set.note} placeholder="Optional set note" onChange={(event) => update({ note: event.target.value })} /></div>;
}
...
function TimerPanel({ timer, setTimer }: { timer?: TimerState; setTimer: (timer?: TimerState) => void }) {
  ...
  const elapsed = timer.pausedAt ? differenceInSeconds(new Date(timer.pausedAt), new Date(timer.startedAt)) - timer.pausedSeconds : differenceInSeconds(new Date(), new Date(timer.startedAt)) - timer.pausedSeconds;
  const remaining = Math.max(0, timer.durationSeconds - elapsed + tick * 0);
  if (remaining === 0 && !timer.skipped && "vibrate" in navigator) navigator.vibrate?.(120);
...
function WalkFormInner({ editing, done }: { editing?: Walk; done: () => void }) {
  const [form, setForm] = useState<Omit<Walk, "id" | "createdAt" | "updatedAt">>({ date: editing?.date ?? todayLocalDate(), time: editing?.time ?? "07:00", distanceKm: editing?.distanceKm ?? 3, durationMinutes: editing?.durationMinutes ?? 35, fastestSplit: editing?.fastestSplit ?? 0, route: editing?.route ?? "", dayPart: editing?.dayPart ?? "morning", setting: editing?.setting ?? "outside", effort: editing?.effort ?? 5, shinDiscomfort: editing?.shinDiscomfort ?? "none", notes: editing?.notes ?? "" });
  async function save() { const payload = { ...form, fastestSplit: form.fastestSplit || undefined, effort: form.effort || undefined, updatedAt: nowIso() }; if (editing) await db.walks.update(editing.id, payload); else await db.walks.add({ id: makeId("walk"), ...payload, createdAt: nowIso() }); done(); }
  return <section className={card}><h2 className="text-xl font-bold">{editing ? "Edit walk" : "Log walk"}</h2><div className="mt-3 grid gap-2 sm:grid-cols-2"><FormInput label="Date" type="date" value={form.date} set={(value) => setForm({ ...form, date: value })} /><FormInput label="Time" type="time" value={form.time} set={(value) => setForm({ ...form, time: value })} /><FormInput label="Distance km" type="number" value={form.distanceKm} set={(value) => setForm({ ...form, distanceKm: Number(value) })} /><FormInput label="Duration minutes" type="number" value={form.durationMinutes} set={(value) => setForm({ ...form, durationMinutes: Number(value) })} /><FormInput label="Route" value={form.route} set={(value) => setForm({ ...form, route: value })} /><select className={input} aria-label="Shin discomfort" value={form.shinDiscomfort} onChange={(event) => setForm({ ...form, shinDiscomfort: event.target.value as Walk["shinDiscomfort"] })}><option value="none">No shin discomfort</option><option value="mild">Mild</option><option value="moderate">Moderate</option><option value="severe">Severe</option></select></div><button className={`${button} mt-3`} onClick={save}>Save walk</button></section>;
}
...
function Habits() {
  ...
  return <HabitsInner key={existing?.id ?? date} date={date} setDate={setDate} existing={existing} />;
}
...
function HabitsInner({ date, setDate, existing }: { date: string; setDate: (date: string) => void; existing?: DailyHabit }) {
  ...
  async function save() { const payload = { date, values, disabled, notes: "", updatedAt: nowIso() }; if (existing) await db.dailyHabits.update(existing.id, payload); else await db.dailyHabits.add({ id: makeId("habit"), ...payload, createdAt: nowIso() }); }
  return <Page title="Habits & sleep" subtitle="Simple adherence, takeout, and sleep tracking with no shaming language."><section className={card}><input className={input} type="date" value={date} onChange={(event) => setDate(event.target.value)} /><div className="mt-3 space-y-2">{HABITS.map((habit) => <div key={habit} className="rounded-2xl bg-white/5 p-3"><label className="flex items-center gap-2"><input type="checkbox" checked={Boolean(values[habit])} disabled={disabled.includes(habit)} onChange={(event) => setValues({ ...values, [habit]: event.target.checked })} /> {habit}</label><button className="mt-1 text-xs text-slate-300" onClick={() => setDisabled(disabled.includes(habit) ? disabled.filter((item) => item !== habit) : [...disabled, habit])}>{disabled.includes(habit) ? "Enable" : "Disable"}</button></div>)}</div><button className={`${button} mt-3`} onClick={save}>Save habits</button></section><SleepAndTakeout /></Page>;
}
...
function ReadinessForm({ sessionId, date }: { sessionId: string; date: string }) { const [form, setForm] = useState({ sleepHours: 6.5, energy: "medium", motivation: "medium", soreness: "mild", currentPain: "none", notes: "" }); async function save() { await db.readinessEntries.add({ id: makeId("ready"), date, sessionId, ...form, createdAt: nowIso(), updatedAt: nowIso() }); } return <section className={card}><h2 className="text-xl font-bold">Readiness</h2><div className="mt-3 grid gap-2"><FormInput label="Sleep hours" type="number" value={form.sleepHours} set={(value) => setForm({ ...form, sleepHours: Number(value) })} /><select className={input} value={form.energy} onChange={(event) => setForm({ ...form, energy: event.target.value })}><option>low</option><option>medium</option><option>high</option></select><FormInput label="Current pain" value={form.currentPain} set={(value) => setForm({ ...form, currentPain: value })} /></div>{form.currentPain !== "none" && <p className="mt-2 text-sm text-amber-200">Train only through pain-free movements. Stop or modify any exercise that reproduces the pain.</p>}<button className={`${ghost} mt-3`} onClick={save}>Save readiness</button></section>; }
...
function Backup() {
  ...
  const walkRows = data.walks.map((walk) => ({
    ...
    pace: formatPace(paceMinutesPerKm(walk.distanceKm, walk.durationMinutes)),
...
function FormInput({ label, value, set, type = "text" }: { label: string; value: string | number; set: (value: string) => void; type?: string }) { return <label className="text-sm text-slate-300">{label}<input className={`${input} mt-1`} type={type} value={value} onChange={(event) => set(event.target.value)} /></label>; }
